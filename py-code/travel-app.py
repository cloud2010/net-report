...
def main():
    ...
    pattern = re.compile(u'饿了么|美团|口碑|点评|外卖|携程|去哪儿|旅行|单车')
    # 数据清洗排除爬虫兴趣标注
    df = df[(df['app名称/类型'].str.contains(pattern) == 1) & (df['统计月份'] == 201710)]
    
    # 按性别分析
    with pd.ExcelWriter('../result/旅游美食APP按性别分类汇总统计.xlsx') as xls:
        for school in ['临港大学城', '杨浦大学城', '闵行大学城', '松江大学城']:
            for sex in ['男', '女']:
                ...
                g_df.to_excel(xls, '{0}_旅游美食_{1}_SEX_APP_T20'.format(school, sex), index=True)
    
    # 按年级分析
    with pd.ExcelWriter('../result/旅游美食APP按年级分类汇总统计.xlsx') as xls:
        for school in ['临港大学城', '杨浦大学城', '闵行大学城', '松江大学城']:
            for age in range(1995, 1999):
                ...
                g_df['月均访问人次'] = g_df['pv'] / int(sum_stu)
                g_df['学生数'] = int(sum_stu)
                g_df.to_excel(xls, '{0}_旅游美食_{1}_AGE_APP_T20'.format(school, age), index=True)
